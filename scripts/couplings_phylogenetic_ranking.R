...
phyl.ranking <- function(coups, BAPS, filt.fasta){
  ...
  # lane IDs
  names.v <- rownames(filt.fasta)

  # fasta to list, each element of list corresponds to a vector of alleles 
  seq.strings <- list()
  for (i in 1:length(filt.fasta[[1]])){
   seq.strings[[i]]=toString(filt.fasta[[1]][i])
  }
  theletters <- list()
...
parser$add_argument("couplings",
                    help="Couplings table")
parser$add_argument("baps",
                    help="BAPS clusters table")
parser$add_argument("alignment",
                    help="Filtered alignment FASTA file (SuperDCA output)")
...
aln = as.data.frame(readDNAStringSet(args$alignment))
baps = read.csv(args$baps)
couplings = read.table(args$couplings, sep='\t')

ranking <- phyl.ranking(couplings, baps, aln)
write.table(ranking, args$output, sep='\t',
	    row.names=F, quote=F)
